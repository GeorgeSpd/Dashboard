...
def multi_tech_selector(
    ...
    n = st.number_input(
        ...
        step=1,
        key=n_key,
        help="Define different types of technologies used to satisfy this particular demand"
    )
...
def render_dhw_section(
    ...
    peakiness  = float(_prof.get("peakiness", 3))
    ...
    with st.expander(title, expanded=False):
        st.caption("Configure DHW profile")

        # Row 1 — core inputs (4 in a row)
        c1, c2 = st.columns(2)
        with c1:
            occ = st.number_input(
                "Occupancy [pers]",
                ...
                value=int(occ),
                step=1,
                key=f"{base_key}_occ_{label_name}",
            )
        with c2:
            lppd = st.number_input(
                "Use [L/pers/day]",
                min_value=0.0,
                value=float(lppd),
                step=1.0,
                key=f"{base_key}_lppd_{label_name}",
            )
        ...
        with c1:
            tap = st.number_input(
                ...
                value=float(tap),
                step=1.0,
                key=f"{base_key}_tap_{label_name}",
            )
        with c2:
            cold = st.number_input(
                ...
                value=float(cold),
                step=1.0,
                key=f"{base_key}_cold_{label_name}",
            )

        with st.expander("Advanced", expanded=False):
            st.caption("Daily Profile Characteristics")
            
            preset = st.radio(
                "Preset",
                options=["Flat", "Morning + Evening", "Evening-heavy"],
                index={"Flat":0, "Morning + Evening":1, "Evening-heavy":2}.get(preset, 1),
                horizontal=False,
            ...
            c1, c2 = st.columns(2)
            with c1:
                peakiness = st.slider("Peakiness", 0.0, 10.0, float(peakiness), 0.1,
                    key=f"{base_key}_peakiness_{label_name}")

            with c2:
                main_hour = st.slider("Main peak hour", 0, 23, int(main_hour), 1,
                    key=f"{base_key}_mainhour_{label_name}")
            ...
            with c1:
                amp = st.slider(
                    "Amplitude",
                    0.0, 0.5, float(amp), 0.01,
                    key=f"{base_key}_season_amp_{label_name}",
                )
            
            with c2:
                peak_month = st.slider(
                    "Peak month",
                    1, 12, int(peak_month), 1,
                    key=f"{base_key}_season_peak_{label_name}",
                )

        # Build UI defaults for multi_tech_selector
        ui_defaults = {}
...
def scenario_controls(label_name: str, defaults: dict | None = None) -> dict:
    ...
    with st.sidebar.expander(expander_title, expanded=False):
        ...
        st.caption("Scenario Definition")
        name = st.text_input("Scenario name", value=current_name, key=name_key, help="Define a scenario name")

        arch = st.selectbox("System architecture", ["Decentralized", "Centralized"], index=0, key=f"arch_{label_name}",
                            help="Define system architecture:\n- Decentralized: Local energy solutions applied to each house separately\n- Centralized: Shared energy solutions applied to all houses")
        ...
        # Comfort
        with st.expander("Demand & Comfort", expanded=False):
            with st.expander("Demand Types", expanded=True):
                dhw_enabled = st.checkbox("DHW enabled", value=_dhw_enabled_default, key=f"dhw_enabled_{label_name}", help="Enable Domestic Hot Water (PS. central heating always on)")
                cooling_enabled = st.checkbox("Cooling enabled", value=defaults.get("cooling_enabled", True), key=f"cooling_enabled_{label_name}", help="Enable central cooling (PS. central heating always on)")
            st.caption("Comfort")
        ...
        st.caption("Equipment",
                   help="Define technology characteristic\n- Decentralized: Definition of local technologies\n- Centralized: Definition of central/shared technologies"
                   )
        heating_chain = []
        cooling_chain = []
        plant_heating_chain = []
