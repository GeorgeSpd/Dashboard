            ...
            if dhw_enabled and "Q_dhw_W" in view.columns and view["Q_dhw_W"].abs().sum() > 0:
                ...
                with c_right:
                    if dhw_enabled and dhw_cols:
                        ...
                        # keep only per-technology series (exclude totals/need/served/unmet)
                        if dhw_enabled:
                            dhw_cols = [
                                c for c in view.columns
                                if c.startswith("Q_dhw_") and c.endswith("_W")
                                and c not in {
                                    "Q_dhw_W",            # delivered total
                                    "Q_unmet_dhw_W",      # unmet total
                                    "Q_dhw_need_W",       # need
                                    "Q_dhw_served_W",     # served total
                                }
                            ]
                        if dhw_enabled and dhw_cols:
                            d_map = {c: c.removeprefix("Q_dhw_").removesuffix("_W")
                                     for c in dhw_cols}
